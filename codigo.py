...
for i in range(len(df)):
    ...
    subj_scr.append(True)
    subj_usscr.append(True)
    subj_con.append(True)
...
tabla = pd.DataFrame(data=[[len(a1)-sum(a1), len(a2)-sum(a2), sum(paisbien)],
                         ...
                         [len(e1)-sum(e1), sum(scr_ok) - len(df) + sum(isna_scr), len(h1)-sum(h1) + len(subj_scr) - sum(subj_scr)],
                         [len(f1)-sum(f1), sum(usscr_ok) - len(df) + sum(isna_usscr), len(h1)-sum(h1) + len(subj_usscr) - sum(subj_usscr)],
                         [len(g1)-sum(g1), sum(consent_ok) - len(df) + sum(isna_consent), len(h1)-sum(h1) + len(subj_con) - sum(subj_con)],
                         [len(h1)-sum(h1) + len(h2)-sum(h2), len(df) - sum(isna_subj) - sum(subj_ok3) , len(h1)-sum(h1) + len(h2)-sum(h2) + sum(paisbien)]])

print(tabla)

reglas = pd.DataFrame(data=[['A1','A2','B1','B2','B3','C1','C2','C3','BC1','D1','D2','E1','E2','F1','F2','G1','G2','H1','H2','H3','H4','H5'],
                         [len(a1)-sum(a1), len(a2)-sum(a2), len(b1)-sum(b1), len(b2)-sum(b2), len(df) - sum(fecha_ok),
                          len(c1)-sum(c1), len(c2)-sum(c2), len(df) - sum(fecha_ok2), len(ok_años_dif) - sum(ok_años_dif),
                          len(d1)-sum(d1), len(d2)-sum(d2), len(e1)-sum(e1), sum(scr_ok) - len(df) + sum(isna_scr),
                          len(f1)-sum(f1), sum(usscr_ok) - len(df) + sum(isna_usscr), len(g1)-sum(g1), sum(consent_ok) - len(df) + sum(isna_consent),
                          len(h1)-sum(h1), len(h2)-sum(h2), len(df) - sum(isna_subj) - sum(subj_ok3), len(set(df.iloc[:,7])) - sum(isna_subj) - 1, sum(paisbien)]])
...
reglas.to_csv('reglas.csv', index=False)